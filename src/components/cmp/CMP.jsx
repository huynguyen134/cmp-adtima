...
const CMP = forwardRef((props, ref) => {
	...
	const checkCMPValid = () => {
		...
		let checkPropertyCheck = Object.values(checkProperty).every(
	...
	const handleChange = (event, checkboxId) => {
		...
		if (checkboxId) {
			...
			checkProperty[checkboxId].error_message = checked
				? ''
				: variablesObj?.[checkProperty[checkboxId].property_name]
						.errorMessage;
		}
		...
		if (value === 'isAcceptByParent') {
			setSelectedCMP(
				selectedCMP.length === termName.length ? [] : termName
			);
			Object.keys(checkProperty).forEach((key) => {
				checkProperty[key].property_value = checked ? true : false;
				checkProperty[key].error_message = checked
					? ''
					: variablesObj?.[checkProperty[key].property_name]
							.errorMessage;
			});
	...
	const callApiConsents = async (userInforId = 0) => {
		try {
			...
			if (postConsentRespone?.statusCode !== 200)
				throw postConsentRespone;
			setShowError('');
	...
	return (
		<div ref={ref} className={classes}>
			{!hideCheckAll && (
				<CmpChild className='cmp-adtima-checkox-all'>
					<CustomCheckbox
						type='checkbox'
					...
					/>
					<CustomCheckboxLabel
						htmlFor='isAcceptByParent'
						className='cmp-adtima-label'
					>
						{term?.name}
					</CustomCheckboxLabel>
				</CmpChild>
			...
			<CmpGroup className='cmp-adtima-group'>
				{term?.term_properties?.map((valueTerm, index) => {
					return (
						<div
							key={`checkbox_${valueTerm._id}`}
							className='cmp-adtima-group-child'
						>
							<CmpChild className='cmp-adtima-child'>
								<CustomCheckbox
									...
									value={valueTerm?._id}
									onChange={(e) =>
										handleChange(e, valueTerm?._id)
									}
									checked={selectedCMP.includes(
										valueTerm?._id
									)}
								/>
								<CustomCheckboxLabel
